...
const TextInput = ({ setUrl, url, loading }: Props) => {
  ...
  return (
    <div className='bg-white h-full w-full min-h-[40px]  flex p-2 rounded-md shadow-sm'>
      <input value={url} required onChange={(e) => setUrl(e.target.value)} type="text " className='bg-transparent h-full w-full outline-none' placeholder='Paste your link' />
      {
        url.trim().length === 0 ?
